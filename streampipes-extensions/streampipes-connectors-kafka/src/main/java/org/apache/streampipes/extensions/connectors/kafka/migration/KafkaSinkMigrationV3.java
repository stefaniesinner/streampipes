...
public class KafkaSinkMigrationV3 implements IDataSinkMigrator {
  ...
  /**
   * Returns the position of the access mode configuration so that the message key configuration
   * is rendered at the same position as for newly created sinks.
   */
  private int indexOfAccessMode(List<StaticProperty> staticProperties) {
    return IntStream.range(0, staticProperties.size())
        .filter(i -> KafkaConfigProvider.ACCESS_MODE.equals(staticProperties.get(i).getInternalName()))
        .findFirst()
